...
export const requestSchema = z.discriminatedUnion('type', [
  ...
  z.object({ type: z.literal('file.list'), solutionId: id, path: nullablePath.default(null) }),
  z.object({
  ...
  z.object({ type: z.literal('source.update'), id, title: z.string().trim().min(1).max(500), mediaType: z.string().max(200).nullable() }),
  z.object({ type: z.literal('source.attach'), projectId: id, id }),
  z.object({ type: z.literal('source.detach'), projectId: id, id }),
  z.object({ type: z.literal('source.classify'), id, solutionId: id }),
  ...
  z.object({
    ...
    state: z.object({
      openTabs: z.array(id).optional(), activeTab: id.optional(),
      readingLocations: z.record(z.string(), z.unknown()).optional(),
      noteDraft: text.nullable().optional(), noteDraftBaseVersion: z.string().max(200).nullable().optional(),
      selectedAnnotationIds: z.array(id).optional(),
      materialWidth: z.number().min(180).max(1_200).optional(), conversationWidth: z.number().min(280).max(1_600).optional(),
      materialTreeCollapsed: z.boolean().optional(),
      pendingNavigation: z.object({ annotationId: id, requestedAt: z.string().max(100) }).nullable().optional(),
    }).refine(value => Object.keys(value).length > 0, '工作现场补丁不能为空。'),
  }),
  ...
  z.object({
    type: z.literal('note.insert'), projectId: id, afterLine: z.number().int().nonnegative(), content: text,
